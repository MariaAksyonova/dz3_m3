document.addEventListener("DOMContentLoaded", function() {
    ...
    button2.addEventListener("click", function() {
        if (button2.style.boxShadow.includes("inset")) {
            button2.style.boxShadow = "14px 15px 11px 3px rgba(15, 79, 128, 0.3)";
        } else {
            button2.style.boxShadow = "inset 5px 5px 10px rgba(15, 79, 128, 0.3)"; ь
        }

        let textElement = button2.querySelector("p");
                if (textElement.textContent === "Ня") {
                    textElement.textContent = "Ау";
                } else {
                    textElement.textContent = "Ня"; 
                }
    });
    });
    });
